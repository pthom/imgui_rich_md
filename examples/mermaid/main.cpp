...
> [!NOTE]
> A subset of Mermaid:
> - **Supported**: flowcharts (shapes, links, labels, subgraphs, the four directions), sequence diagrams
>   (participants, actors, activations, notes, frames, autonumber), class diagrams (members, relations, cardinalities,
>   namespaces, notes, generics).
> - **Not supported**: the other diagram types (state, ER, gantt, pie, git graphs, mind maps...), links to a subgraph,
>   `direction` inside a subgraph. Styles (`classDef`, `style`) and `click` are ignored.
>
